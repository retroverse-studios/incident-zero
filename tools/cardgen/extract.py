...
HEADING_RE = re.compile(r"^#{3,4} (?:Card )?([A-Z]{1,5}-\d{2,3}): (.+)$")
BOX_EDGE = "┌┐└┘╔╗╚╝"
BOX_SEP = "├╠"
...
def _parse_box(box_lines):
    ...
    card_type = segments[0][0].title() if segments else ""
    fields, sections = {}, []
    ...
    for seg in segments[2:] if len(segments) > 2 else []:
...
def parse_file(path, default_type=""):
    ...
    while i < len(lines):
        ...
        if has_box:
            ...
            fields, sections = _parse_fields(block)
        cards.append({
            "id": card_id,
            "title": title,

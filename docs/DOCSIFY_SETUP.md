...
> **Note (v2.2):** the docsify app now lives at **`docs.html`** — the root `index.html` is the game's landing page (downloads + playtester call-to-action). The landing page forwards legacy `#/...` deep links to `docs.html` automatically. Print-and-play bundles in `downloads/` are generated by `tools/build_bundles.py`.
